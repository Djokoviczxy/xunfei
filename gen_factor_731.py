...
def gen_factor_train(date, code):
    ...
    # , 'min', 'skew', 'kurt', 'qtlu', 'qtld', 'rank', 'imax', 'imin'
    # 扩充特征——生成的特征
    ...
    if isExists:
        ...
        factor_am = np.concatenate(
            [factor_am0, factor_am1, factor_am2, factor_am3, factor_am4, factor_am5], axis=1)
        factor_am = np.concatenate([am_index.values, factor_am],axis=1)
        ...
        # 释放内存
        del data_am, factor_am, factor_am0, factor_am1, factor_am2, factor_am3, factor_am4, factor_am5
        gc.collect()
    ...
    if isExists:
        ...
        factor_pm = np.concatenate([pm_index.values, factor_pm], axis=1)
        ...
        # 释放内存
        del data_pm, factor_pm, factor_pm0, factor_pm1, factor_pm2, factor_pm3, factor_pm4, factor_pm5
        gc.collect()
...
# j_num = 2
start = time.time()
results_train_1 = Parallel(n_jobs=-1)(delayed(gen_factor_train)(i, j) for i in tqdm(range(num), position=0) for j in range(2,4))
end = time.time()
print('{:.4f} s'.format(end-start))
